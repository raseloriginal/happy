...
<body class="h-full flex flex-col justify-between overflow-hidden">
  ...
  <nav class="bg-gray-100 shrink-0 flex items-center justify-around border-t border-[#cbd5e1] z-40 select-none">
    <button onclick="location.href='index.php'" id="nav-home" class="flex items-center justify-center gap-2 text-gray-600 py-3.5 flex-1 font-mono text-[11px] font-bold border-r border-[#cbd5e1] border-b border-gray-100 btn-bounce">
    ...
    <button onclick="location.href='settlement.php'" id="nav-settlement" class="flex items-center justify-center gap-2 text-gray-600 py-3.5 flex-1 font-mono text-[11px] font-bold border-b border-gray-100 btn-bounce tab-active">
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      // Generate Banknote Grid
      renderBanknotes();

      // Fetch dispatch details and perform initial calculations
      loadDashboard(localISOTime).then(() => calcSettlementExpected());
    });
    ...
    // Fetch Dashboard Data (used here to retrieve active dispatch details for calculation)
