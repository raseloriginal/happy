<?php
// eggland/order_edit.php — Edit Order (Excel Spreadsheet Style)

require_once __DIR__ . '/data.php';

$pageTitle = 'Eggland Edit Order Billing';
...
if ($orderId <= 0) {
    header('Location: ' . rootPath() . '/eggland/orders.php');
    exit;
}

$order = eggGetOrder($orderId);
if (!$order) {
    header('Location: ' . rootPath() . '/eggland/orders.php');
    exit;
}
...
// Map products for validation lookup
$productMap = [];
    ...
    if (!$agent) {
        ...
        if (empty($err)) {
            if (empty($orderItems)) {
                ...
                if ($updated && eggSaveOrders($orders)) {
                    // Redirect to orders ledger list with success msg
                    header('Location: ' . rootPath() . '/eggland/orders.php?msg_success=2');
                    exit;
                } else {
                    $err = 'Failed to update order invoice details inside JSON database.';
                }
            }
        }
    }
    ...
    <div class="page-body">
      ...
      <div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between border-b border-gray-200 pb-3 gap-2">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <span class="text-excel-green"><i class="fa-solid fa-edit"></i></span> 
            Edit Egg Order: Invoice #<?= str_pad($order['id'], 4, '0', STR_PAD_LEFT) ?>
          </h2>
          <p class="text-xs text-gray-500 font-mono">Operations Mode: Interactive Order Adjustment Sheet</p>
        </div>
        <div>
      ...
      <form action="" method="POST" id="order-form" onsubmit="return validateOrderSheet()">
        ...
        <div class="bg-white border border-gray-200 p-4 mb-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select name="agent_id" class="excel-input select" required>
                ...
                <?php foreach ($agents as $a): 
                  $selected = (intval($a['id']) === intval($order['agent_id'])) ? 'selected' : '';
                ?>
                  <option value="<?= $a['id'] ?>" <?= $selected ?>><?= htmlspecialchars($a['name']) ?> (<?= htmlspecialchars($a['phone']) ?>)</option>
                <?php endforeach; ?>
              </select>
            </div>
            ...
            <div>
              ...
              <input type="date" name="order_date" class="excel-input" value="<?= htmlspecialchars($order['order_date']) ?>" required>
            </div>
            
            <div>
              <label class="form-label">Accountant Remarks / Shipping Notes</label>
              <input type="text" name="notes" class="excel-input" value="<?= htmlspecialchars($order['notes']) ?>" placeholder="e.g. Morning delivery, collect on bank DBBL transfer">
            </div>
          </div>
        </div>
          ...
          <div class="excel-grid-container">
            <table class="excel-table" id="items-grid-table">
              ...
              <tbody id="items-grid-body">
                <!-- Pre-filled items will be loaded by Javascript -->
              </tbody>
              <tfoot>
                <tr class="bg-gray-50 font-bold border-t-2 border-gray-300">
                  ...
                  <td class="text-right text-excel-green font-mono text-base border-r border-gray-200" id="grand-total-display">৳ <?= number_format($order['total_amount'], 2) ?></td>
                  <td>&nbsp;</td>
                </tr>
              </tfoot>
            </table>
        ...
        <div class="flex justify-end gap-3 pb-6">
          <a href="<?= rootPath() ?>/eggland/orders.php" class="btn-excel-ghost py-2.5 px-5">Cancel Editing</a>
          <button type="submit" class="btn-excel py-2.5 px-6 font-bold"><i class="fa-solid fa-save"></i> Save Adjustments</button>
        </div>
...
function appendItemRow(prefilledProductId = "", prefilledQty = 1) {
    rowCounter++;
    ...
    productsCatalog.forEach(p => {
        const isSel = (p.id === prefilledProductId) ? 'selected' : '';
        optionsHtml += `<option value="${p.id}" data-price="${p.price}" data-unit="${p.unit}" ${isSel}>${p.name} (৳${p.price.toFixed(2)})</option>`;
    });
    ...
    tr.innerHTML = `
        ...
        <td>
            <input type="number" name="item_quantity[]" class="excel-input text-center font-mono input-qty" id="qty-${rowCounter}" value="${prefilledQty}" min="1" required oninput="calculateRow(${rowCounter})">
        </td>
    ...
    // Recalculate row details if prefilled product is selected
    if (prefilledProductId !== "") {
        onProductSelect(rowCounter);
    } else {
        reindexRows();
    }
}
...
// Initialise and pre-load all existing items on startup!
document.addEventListener("DOMContentLoaded", function() {
    const existingItems = <?= json_encode($order['items']) ?>;
    if (existingItems && existingItems.length > 0) {
        existingItems.forEach(item => {
            appendItemRow(item.product_id, item.quantity);
        });
    } else {
        appendItemRow();
    }
