  ...
  <div class="main-content">
    ...
    <div class="page-body">
      <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?= rootPath() ?>/manager/lot_add.php" class="btn btn-primary">+ Add Lot</a>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="data-table">
          ...
          <tbody>
            ...
            <?php endforeach; ?>
            <?php if (empty($lots)): ?><tr><td colspan="7" class="text-center py-8 text-gray-400">No lots yet. <a href="<?= rootPath() ?>/manager/lot_add.php" class="text-indigo-600">Add one</a></td></tr><?php endif; ?>
          </tbody>
        </table>
      </div>
    </div>
...
async function deleteLot(id) {
  ...
  const data = await api('<?= rootPath() ?>/api/lots.php?id=' + id, 'DELETE');
