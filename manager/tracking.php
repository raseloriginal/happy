...
// Get Warehouse Name
$whStmt = $pdo->prepare('SELECT name FROM warehouses WHERE id = ?');
$whStmt->execute([$wid]);
$warehouseName = $whStmt->fetchColumn() ?: 'Your Warehouse';
  ...
  <div class="main-content">
    ...
    <div class="page-body">
      ...
      <div class="flex flex-wrap items-center justify-between mb-6 gap-3">
        <div>
          <h2 class="text-xl font-bold text-gray-800"><i class="fa-solid fa-map-location-dot text-indigo-500 mr-2"></i>DSR Real-time Tracking</h2>
          <p class="text-sm text-gray-500">Live locations of Delivery Sales Representatives for <strong><?= htmlspecialchars($warehouseName) ?></strong></p>
        </div>
        ...
        <div class="lg:col-span-1 flex flex-col gap-4">
          <div class="bg-white p-4 border border-gray-100 rounded-xl shadow-sm">
            <div class="relative">
              ...
              <input type="text" id="dsrSearch" placeholder="Search DSR by name..." class="w-full bg-gray-50 border border-gray-200 py-2 pl-9 pr-4 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:bg-white transition-all" oninput="filterDsrList()" />
            </div>
          </div>
...
function renderDsrList() {
  ...
  const filtered = dsrData.filter(d => d.name.toLowerCase().includes(searchVal));
    ...
    if (d.latitude !== null && d.longitude !== null) {
      ...
      const popupContent = `
        <div style="font-family:'Inter', sans-serif; padding:2px; min-width:140px;">
          <h4 style="margin:0 0 4px 0; font-weight:700; color:#1f2937; font-size:13px;">${esc(d.name)}</h4>
